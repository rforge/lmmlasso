...
function(object,...)
 {
  # 1. part
    ...
    cat("family =",object$family,"; ntot =",object$ntot,"; N =",object$N,"; p =",object$p,"; lambda =",round(object$lambda,3),"\n")
    print(table1)
    ...
    if (is.null(xnames))
	{
	  rownames(table3) <- c("(Intercept)",paste("X", 2:length(object$fixef),sep=""))
	} else
	{
...
function(x,...)
  {
    ...
    names(table1) <- c("AIC","BIC","logLik","deviance","objective")
    cat("Generalized linear mixed model fit by the Laplace approximation","\n")
    cat("family =",x$family,"; ntot =",x$ntot,"; N =",x$N,"; p =",x$p,"; lambda =",round(x$lambda,3),"\n")
    print(table1)
...
function(x,...)
 {
   ...
   plot(x$workResid~x$eta,col=x$data$group,main="Tukey-Anscombe Plot",xlab="eta",ylab="working residuals")
   abline(h=0,col="grey")

   if (x$family=="binomial")
    {
     plot(x$respResid~x$mu,col=x$data$group,main="Tukey-Anscombe Plot",xlab="mu",ylab="response residuals")
     abline(h=0,col="grey")
    } else
   {
     plot(x$data$y~x$fitted,col=x$data$group,main="Tukey-Anscombe Plot",xlab="Predicted values",ylab="Observed values")
   }
