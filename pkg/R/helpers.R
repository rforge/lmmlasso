ArmijoRule <-
function(xGroup,yGroup,LGroup,b,j,cut,HkOldJ,HkJ,JinNonpen,lambda,weights,nonpen,ll1,ll2,converged,control)
{
 b.new <- b
 bJ <- b[j]
 grad <- -cut + bJ*HkOldJ
     ...
     for (l in 0:control$max.armijo)
      { 
         ...
         if (l==control$max.armijo)
           {
             if (trace>1) cat("Armijo for b_",j," not successful","\n")
...
HessianMatrix <-
function(xGroup,LGroup,activeSet,N,hessian,mat)
{
  for (i in 1:N)
    {  
...
MLloglik <-
function(xGroup,yGroup,LGroup,b,ntot,N,activeSet)
{
...
MLpdDiag <-
function(zGroup,resGroup,zIdGroup,sigma,pars,q,thres=10^(-4),ll1,ll4,trace,opt,VarInt)
{
...
MLpdIdentFct <-
function(thetak,zGroup,zIdGroup,resGroup,sigma,LPsi)
{
  Psi <- thetak^2*LPsi
...
MLpdSym <-
function(zGroup,resGroup,zIdGroup,sigma,pars,q,thres=10^(-4),ll1,ll4,trace,opt,CovInt,VarInt)
{
...
MLpdSymFct <-
function(thetak,zGroup,zIdGroup,resGroup,sigma,a,b,LPsi)
{
  LPsi[a,b] <- thetak
  Psi <- tcrossprod(LPsi)
  LambdaGroup <- mapply(MLpdSymLambda,Z=zGroup,ZId=zIdGroup,MoreArgs=list(sigma=sigma,Psi=Psi))
...
MLpdSymObj <-
function(Lambda,res)  determinant(Lambda)$modulus + MyQuadFormInv(Lambda,res)

MLsigma <-
function(zGroup,zIdGroup,resGroup,q,ll1,ll4,true.sigma,Psi,trace,opt,VarInt)
{
...
MedianValue <-
function(grad,hessian,lambda,bj)
 {
...
ObjFunction <-
function(xGroup,yGroup,LGroup,b,weights,lambda,nonpen,resGroup=NULL,ll1=ll1,ll2=NULL)
{
...
as1 <-
function(xGroup,LGroup,activeSet,N)
  {
...
as2 <-
function(x,y,b,j,activeSet,group,sGroup)
{
...
covStartingValues <-
function(xGroup,yGroup,zGroup,zIdGroup,b,ntot,N,lower=-10,upper=10)
{
  ...
  optRes <- optimize(f=optimize4,interval=c(lower,upper)) # interval=[4.539993e-05,22026.47]
