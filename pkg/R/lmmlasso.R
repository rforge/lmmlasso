...
lmmlasso.default <-
function(x,y,z=x,grp,weights=NULL,coefInit=NULL,lambda,startValue=1,nonpen=1:dim(z)[[2]],pdMat=c("pdIdent","pdDiag","pdSym"),
                       method="ML",opt=c("optimize","nlminb"),stopSaturated=TRUE,standardize=TRUE,control=lmmlassoControl(),...)
{
...
if (missing(weights))
 {
   weights <- rep(1,dim(x)[[2]])
 } else
    {
      ...
      random <- 1:dim(z)[[2]]
      rem <- which(weights==0)
      z <- z[,!(random%in%rem),drop=FALSE]

      weights <- weights[!remove]

   }

if (standardize) x <- cbind(1,scale(x[,-1]))
...
if (control$trace>2) cat("fctStart:",fctStart,"\n")
...
while ((control$maxIter > counter) &  (convPar > control$tol | convFct > control$tol | convCov > control$tol | !doAll ))
 {
  if (control$maxIter==counter+1) {if (control$trace>1) cat("maxIter reached","\n") ; converged <- converged + 1}
  ...
  if ((length(activeSet)>min(p,ntot))&(lambda>0)&(stopSaturated)&(counter>2)) {stopped <- TRUE ; break}
  ...
  for (j in activeSet)
   {
    ...
    if (control$trace>2) {fctIter <- ObjFunction(xGroup=xGrp,yGroup=yGrp,LGroup=lambdaInvGrp,b=betaIter,weights=weights,
                        lambda=lambda,nonpen=nonpen,ll1=ll1,ll2=ll2) ; print(fctIter) }
   } 

  if (control$trace>2) cat("-----------","\n")
  ...
  if (pdMat=="pdSym")
   { 
     covParOpt <- MLpdSym(zGroup=zGrp,zIdGroup=zIdGrp,resGroup=resGrp,sigma=sIter,pars=parsIter,q=q,
                      thres=control$thres,ll1=ll1,ll4=ll4,trace=control$trace,opt=opt,VarInt=control$VarInt,CovInt=control$CovInt)
  ...
  else if (pdMat=="pdDiag")
    {
     covParOpt <- MLpdDiag(zGroup=zGrp,zIdGroup=zIdGrp,resGroup=resGrp,sigma=sIter,pars=parsIter,q=q,
                       thres=control$thres,ll1=ll1,ll4=ll4,trace=control$trace,opt=opt,VarInt=control$VarInt)
  ...
  covParOpt <- MLsigma(zGroup=zGrp,zIdGroup=zIdGrp,resGroup=resGrp,q=q,ll1=ll1,ll4=ll4,true.sigma=sIter,Psi=PsiIter,
                     trace=control$trace,opt=opt,VarInt=control$VarInt)
...
if (stopped) {cat("|activeSet|>=min(p,ntot): Increase lambda or set stopSaturated=FALSE.","\n")
...
out <- list(data=list(x=x,y=y,z=z,grp=grp),weights=weights,coefInit=list(betaStart=betaStart,parsStart=parsStart,sStart=sStart),
                 lambda=lambda,sigma=sIter,pars=parsIter,coefficients=betaIter,random=bi,u=u,ranef=ranef,fitted.values=fitted,
                 residuals=residuals,Psi=Psi,corPsi=corPsi,converged=converged,logLik=logLik,npar=npar,deviance=deviance,
                 aic=aic,bic=bic,nonpen=nonpen,counter=counter,pdMat=pdMat,method=method,opt=opt,control=control,call=match.call(),
                 stopped=stopped)
